...
SELECT id
     ...
     , CAST(c.y1 AS TEXT) || '-' || CAST(cs.min_year AS TEXT) || '=' || CAST(c.y1-cs.min_year AS TEXT) 'min_year:c-cs'
     , CAST(c.y2 AS TEXT) || '-' || CAST(cs.max_year AS TEXT) || '=' || CAST(c.y2-cs.max_year AS TEXT) 'max_year:c-cs'
  FROM colors c
  JOIN color_stats cs
    ON c.id = cs.color_id
